...
internal static class SurfAI
{
    ...
    private static void ExplainCloudError(string prompt)
    {
        ...
        {
            Console.WriteLine($"That error means the package requires a newer SurfOS version. This build reports SurfOS {CloudRepositoryManager.SurfOsVersion}.");
        }
    ...
    private static bool IsSupportedSurfOsVersion(string minimumVersion)
    {
        if (string.IsNullOrWhiteSpace(minimumVersion) ||
            ...
            !Version.TryParse(CloudRepositoryManager.SurfOsVersion, out Version? current))
        {
            return true;
        }
