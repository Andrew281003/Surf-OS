...
internal static class PackageBuilder
{
    ...
    private const int MaximumSourceSnapshotBytes = 32 * 1024 * 1024;
        ...
        out string error)
    {
        ...
        File.WriteAllText(
            Path.Combine(projectPath, ".surfignore"),
            "# Files excluded from the optional SurfCloud source snapshot\n" +
            ".git/\n.exports/\nbin/\nobj/\n*.pdb\n*.user\n.env\n.env.*\nsecrets.json\n");
        return true;
    }
    ...
    internal static bool TryCreateSourceSnapshot(string id, out byte[] snapshot, out string error)
    {
        snapshot = [];
        error = string.Empty;
        if (!TryGetProjectPath(id, out string projectPath))
        ...
        try
        {
            SurfIgnore ignore = SurfIgnore.Load(Path.Combine(projectPath, ".surfignore"));
            ...
            using (ZipArchive archive = new(output, ZipArchiveMode.Create, leaveOpen: true))
            {
                ...
                foreach (string file in Directory.EnumerateFiles(projectPath, "*", SearchOption.AllDirectories))
                {
                    ...
                    if (totalBytes > MaximumSourceSnapshotBytes)
                    {
                        error = "Source snapshot exceeds the 32 MB upload limit. Add build output to .surfignore.";
                        return false;
                    }
        ...
        {
            error = $"Source snapshot failed: {ex.Message}";
            return false;
        }
    }
    ...
    internal static bool TryGetProjectPath(string id, out string path)
    {
        path = string.Empty;
        id = NormalizeId(id);
...
internal sealed class SurfIgnore
{
    private readonly List<(string Pattern, bool Negated)> _rules;

    private SurfIgnore(List<(string Pattern, bool Negated)> rules) => _rules = rules;

    public static SurfIgnore Load(string path)
    {
        if (!File.Exists(path)) return new SurfIgnore([]);
        List<(string Pattern, bool Negated)> rules = [];
        foreach (string rawLine in File.ReadLines(path))
        {
            string line = rawLine.Trim();
            if (line.Length == 0 || line.StartsWith('#')) continue;
            bool negated = line.StartsWith('!');
            string pattern = (negated ? line[1..] : line).Trim().Replace('\\', '/').TrimStart('/');
            if (pattern.Length > 0) rules.Add((pattern, negated));
        }
        return new SurfIgnore(rules);
    }
    ...
    public bool IsIgnored(string relativePath)
    {
        ...
        bool ignored = false;
        foreach ((string pattern, bool negated) in _rules)
        {
            if (Matches(path, pattern)) ignored = !negated;
        }
        return ignored;
    }
    ...
    private static bool Matches(string path, string pattern)
    {
        ...
        string normalized = pattern.TrimEnd('/');
        if (normalized.Length == 0) return false;

        if (!normalized.Contains('/'))
        {
            return path.Split('/').Any(segment => FileSystemName.MatchesSimpleExpression(
                normalized, segment, ignoreCase: true));
        }

        if (directoryPattern && (path.Equals(normalized, StringComparison.OrdinalIgnoreCase) ||
                                 path.StartsWith(normalized + "/", StringComparison.OrdinalIgnoreCase)))
        {
            return true;
        }

        return FileSystemName.MatchesSimpleExpression(normalized, path, ignoreCase: true);
    }
}
