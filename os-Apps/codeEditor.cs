...
namespace SurfOS2.os_Apps
{
    public class CodeEditor
    {
        ...
        private static void CreateProject()
        {
            ...
            {
                Console.WriteLine("Project name cannot be blank.");
                Pause();
                return;
            }
        ...
        {
            StopRunningProjectExecutable(projectRoot, showMessage: true);
        ...
        private static void RunDotnet(string projectRoot, string command, bool openDiagnostics)
        {
            ...
            StopRunningProjectExecutable(projectRoot, showMessage: false);
        ...
        private static bool TryShowLockedProcessDiagnostic(string output)
        {
            ...
            if (int.TryParse(match.Groups["pid"].Value, out int pid) &&
                TryTerminateProcess(pid, out string stopMessage))
            {
                Console.ForegroundColor = ConsoleColor.Green;
                Console.WriteLine(stopMessage);
                Screen_Print.ResetColors();
                Console.WriteLine("Try Build or Debug again.");
            }
            else
            {
        ...
        private static void StopRunningProjectExecutable(string projectRoot, bool showMessage)
        {
            string processName = GetProjectExecutableName(projectRoot);
            ...
            foreach (Process process in Process.GetProcessesByName(processName))
            {
                try
                {
                    ...
                    {
                        Console.ForegroundColor = ConsoleColor.Yellow;
                        Console.WriteLine($"Stopped previous {processName} process ({pid}).");
                        Screen_Print.ResetColors();
                    }
                }
                catch
                {
                    ...
                    {
                        Console.ForegroundColor = ConsoleColor.Yellow;
                        Console.WriteLine($"Could not stop an existing {processName} process.");
                        Screen_Print.ResetColors();
                    }
                }
                finally
        ...
        private static string GetProjectExecutableName(string projectRoot)
        {
            string? projectFile = Directory.GetFiles(projectRoot, "*.csproj").FirstOrDefault();
            return projectFile is null ? string.Empty : Path.GetFileNameWithoutExtension(projectFile);
        }

        private static bool TryTerminateProcess(int pid, out string message)
        {
            try
            {
                ...
                process.Kill(entireProcessTree: true);
                process.WaitForExit(1500);
                message = $"Stopped locked process {processName} ({pid}).";
                return true;
            }
            catch (Exception ex)
            {
                message = $"Could not stop process {pid}: {ex.Message}";
                return false;
            }
        }
