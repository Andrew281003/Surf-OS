    ...
    internal partial class CLI_Engine
    {
        ...
        private static readonly IReadOnlyDictionary<string, (string Usage, string Details)> CommandUsage =
            new Dictionary<string, (string, string)>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                ["app"] = ("app --list | app --open <name> | app --config", "List, launch, or inspect the configuration of installed SurfOS apps."),
                ...
                ["check"] = ("check --system | check --app <name>", "Run read-only health checks for SurfOS or one installed app."),
                ...
                ["surf"] = ("surf <store|search [name]|install <package>|remove <package>|list|update|updates|info <package>>", "Manage SurfCloud packages."),
        ...
        public static void StartTerminal()
        {
            try
            {
                ...
                bool isRunning = true;
                StringBuilder screenTranscript = new();

                while (isRunning)
                {
                    RetroConsole.ApplyForegroundColor(ConsoleColor.Green);

                    string prompt;
                    if (Import.Variables.activePromptStyle == "Linux") prompt = $"{Import.Variables.userName}:~$ ";
                    else if (Import.Variables.activePromptStyle == "Minimal") prompt = "> ";
                    else if (Import.Variables.activePromptStyle == "Hacker")
                    {
                        RetroConsole.ApplyForegroundColor(ConsoleColor.DarkRed);
                        prompt = "root@sys# ";
                    }
                    else
                    {
                        ...
                        prompt = $"{rankLabel}{Import.Variables.userName}@SurfOS> ";
                    }

                    Console.Write($"\n{prompt}");
                    Screen_Print.ResetColors();

                    string input = RetroConsole.ReadShellInput(History);
                    AppendScreenTranscript(screenTranscript, $"\n{prompt}{input}\n");

                    if (IsVimInvocation(input))
                    {
                        ExecuteCommand(input, ref isRunning, false);
                        RestoreScreenTranscript(screenTranscript);
                        continue;
                    }

                    TextWriter originalOutput = Console.Out;
                    StringBuilder commandOutput = new();
                    Console.SetOut(new TranscriptTextWriter(originalOutput, commandOutput));
                    try
                    {
                    ...
                    if (IsClearInvocation(input)) screenTranscript.Clear();
                    else AppendScreenTranscript(screenTranscript, commandOutput.ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private static bool IsVimInvocation(string input) =>
            FirstCommandWord(input) is "vim" or "edit";

        private static bool IsClearInvocation(string input) =>
            FirstCommandWord(input) is "clear" or "cleart";

        private static string FirstCommandWord(string input)
        {
        ...
        private static void AppendScreenTranscript(StringBuilder transcript, string text)
        {
            const int maximumCharacters = 64 * 1024;
            transcript.Append(text);
            if (transcript.Length <= maximumCharacters) return;

            int removeCount = transcript.Length - maximumCharacters;
            int nextLine = transcript.ToString().IndexOf('\n', removeCount);
            transcript.Remove(0, nextLine >= 0 ? nextLine + 1 : removeCount);
        }

        private static void RestoreScreenTranscript(StringBuilder transcript)
        {
            string[] lines = transcript.ToString()
                .Replace("\r\n", "\n", StringComparison.Ordinal)
                .Split('\n');
            int availableRows = Math.Max(1, Console.WindowHeight - 2);
            string visibleScreen = string.Join(Environment.NewLine, lines.TakeLast(availableRows)).TrimEnd();

            Console.Clear();
            if (visibleScreen.Length > 0) Console.Write(visibleScreen);
        }

        private sealed class TranscriptTextWriter(TextWriter inner, StringBuilder transcript) : TextWriter
        {
            ...
            {
                inner.Write(value);
                if (value is not null) transcript.Append(value);
            }
        ...
        private static void ExecuteSimpleCommand(string[] commandParts, ref bool isRunning, bool isScriptExecution)
        {
            ...
            try
            {
                switch (mainCommand)
                {
                    ...
                    case "check":
                        RunHealthCheck(commandParts, cmdIndex);
                        break;
                    ...
                    case "app":
                        ManageApps(commandParts, cmdIndex, ref isRunning, isScriptExecution);
                        break;
            ...
            var sections = new (string Key, string Title, (string Command, string Description)[] Commands)[]
            {
                ...
                ("files", "Files", [
                 ...
                 ("apps", "Apps & Productivity", [
                     (UsageOf("app"), "Lists, opens, and shows configuration for installed apps"),
                ...
                ("system", "System & Diagnostics", [
                    (UsageOf("check"), "Checks system or installed-app health"),
            ...
            bool isScriptExecution)
        {
            return command switch
            {
                ...
                "edit" or "vim" => args.Length > cmdIndex + 1,
