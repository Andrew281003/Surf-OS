...
internal partial class CLI_Engine
{
    private static readonly Dictionary<string, (string Usage, string Details)> AdditionalUsage = new()
    {
        ...
        ["chmod"] = ("chmod <mode> <path>", "Persist a three-digit octal virtual mode; file write bits also control the Windows read-only attribute."),
        ...
        ["hostname"] = ("hostname [new-name --force]", "Display or persist the SurfOS machine name, without changing Windows."),
        ...
        ["ip"] = ("ip [addr|route|status]", "Read host interface addresses and default gateways; does not change Windows networking."),
        ...
        ["fsck"] = ("fsck [path]", "Read-only virtual traversal and file-length check; does not repair NTFS."),
        ...
        ["publish"] = ("publish <build> [--public|--private]", "Validate a build; uploading requires an unavailable SurfCloud publishing backend."),
        ["sign"] = ("sign <package> | sign verify <package>", "Create or verify a detached RSA-SHA256 signature using a local Windows signing key.")
    };
    ...
    private static bool TryExecuteAdditional(string command, string[] args, bool forced, ref bool running)
    {
        ...
        try
        {
            switch (command)
            {
                ...
                case "reboot":
                    Require(args.Length == 0, usage);
                    if (!OperatingSystem.IsWindows()) { BackendUnavailable("SurfOS Windows boot sequence"); break; }
                ...
                case "fsck":
                    ...
                    Console.WriteLine($"Virtual traversal passed: {entries} entries, {bytes} file bytes. Read-only; NTFS integrity is not tested.");
                    break;
                case "ln":
                ...
                case "publish":
                    Require(args.Length == 1 || args.Length == 2 && args[1] is "--public" or "--private", usage);
                    if (ValidateBuild(args[0])) BackendUnavailable("SurfCloud publishing"); break;
    ...
    private static void ExecuteKernel(string[] args)
    {
        ...
        if (action == "verify")
        {
            string assembly = typeof(CLI_Engine).Assembly.Location;
            Console.WriteLine($"Runtime SHA-256: {Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(File.ReadAllBytes(assembly)))}");
