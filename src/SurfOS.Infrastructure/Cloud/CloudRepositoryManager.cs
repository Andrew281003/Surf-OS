...
internal sealed class PackageUpdateNotificationState
{
    public Dictionary<string, string> LastNotifiedVersions { get; set; } =
...
internal static class CloudRepositoryManager
{
    ...
    public static List<PackageUpdate> GetAvailableUpdates(StoreManifest manifest)
    {
        InstalledStorePackageState installed = LoadInstalledState();
        return installed.Packages
            .Select(local =>
            {
                StorePackageManifest? remote = manifest.Packages.FirstOrDefault(package =>
                    package.Id.Equals(local.Id, StringComparison.OrdinalIgnoreCase));
                return remote is not null && CompareVersions(remote.Version, local.Version) > 0
                    ? new PackageUpdate(local.Id, local.Name, local.Version, remote.Version)
                    : null;
            })
    ...
    public static List<PackageUpdate> GetNewUpdateNotifications(StoreManifest manifest)
    {
        EnsureDirectories();
        string statePath = Path.Combine(GetAppsPath(), "update_notifications.json");
        PackageUpdateNotificationState state = File.Exists(statePath)
            ? JsonStorage.Read<PackageUpdateNotificationState>(statePath) ?? new()
            : new();
        state.LastNotifiedVersions = new Dictionary<string, string>(
            state.LastNotifiedVersions ?? new Dictionary<string, string>(),
            StringComparer.OrdinalIgnoreCase);

        List<PackageUpdate> notifications = [];
        foreach (PackageUpdate update in GetAvailableUpdates(manifest))
        {
            if (!state.LastNotifiedVersions.TryGetValue(update.Id, out string? notifiedVersion) ||
                !notifiedVersion.Equals(update.AvailableVersion, StringComparison.OrdinalIgnoreCase))
            {
                notifications.Add(update);
                state.LastNotifiedVersions[update.Id] = update.AvailableVersion;
            }
        }

        JsonStorage.Write(statePath, state);
