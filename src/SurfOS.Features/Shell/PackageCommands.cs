...
internal partial class CLI_Engine
{
    ...
    private static void ExecuteSign(string[] args)
    {
        ...
        if (!OperatingSystem.IsWindows()) { BackendUnavailable("Windows signing key storage"); return; }
        // The private key stays non-exportable in the current Windows user's CNG key store.
        ...
        using CngKey key = CngKey.Exists(keyName) ? CngKey.Open(keyName) : CngKey.Create(CngAlgorithm.Rsa, keyName,
            new CngKeyCreationParameters { ExportPolicy = CngExportPolicies.None, KeyUsage = CngKeyUsages.Signing, Parameters = { new CngProperty("Length", BitConverter.GetBytes(3072), CngPropertyOptions.None) } });
        using RSACng signer = new(key);
