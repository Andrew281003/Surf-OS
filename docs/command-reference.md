...
publish wave-runner --private
surf updates
check --system
check --app surfcode-ide
app --list
app --open surfcode-ide
app --config
```

`check --system` verifies the installation directory, core virtual filesystem
layout, storage writes, active session, JSON state files, and service health.
`check --app <name>` accepts an installed package ID or display name and verifies
its package registration and installed files. Apps that are stored directly under
`/apps` are also recognized. An app does not have to be open to be healthy; the
report states separately whether it currently has a running process. A healthy
check exits with status 0, an unhealthy check with status 1, and invalid usage
with status 2.

`app --list` shows installed non-theme packages and whether each package defines
a launch command. `app --open <name>` accepts either a package ID or its quoted
display name and launches the command declared by that package. `app --config`
shows the app registry locations and current SurfCloud/network settings without
changing them.
