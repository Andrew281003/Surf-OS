...
internal static class Install_Setup
{
    ...
    public static void Install_WizardP1()
    {
        ...
        RetroConsole.TypeLine(
            OperatingSystem.IsWindows()
                ? "CREATING SURFOS SYSTEM DRIVE"
                : "CREATING SURFOS SYSTEM VOLUME",
            4);
        Console.ResetColor();
        if (!Partition_Manager.ProvisionSystemStorage(out string storageError))
        {
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine($"\nSurfOS could not create its system drive:\n{storageError}");
            Console.ResetColor();
            Console.WriteLine("No changes were made to the host partition table.");
            Console.WriteLine("Press any key to exit setup.");
            Console.ReadKey(intercept: true);
            return;
        }

        if (!File_Setup_Manager.Main_Directory())
        {
            return;
        }
    ...
    private static bool SelectInstallDirectory(int step, int totalSteps)
    {
        ...
        try
        {
            if (useVhdx)
            {
                Import.Variables.vhdxHostDirectory = selectedDirectory;
                Import.Variables.partitionDriveLetter = Partition_Manager.GetPreferredDriveLetter();
                Import.Variables.vhdxPath = Path.Combine(selectedDirectory, "SurfOS-System.vhdx");
                Import.Variables.installPath = $"{Import.Variables.partitionDriveLetter}:\\";
            }
            else
            {
                Import.Variables.installPath = Path.GetFullPath(selectedDirectory);
    ...
    private static void ConfigurePartition(int step, int totalSteps)
    {
        ...
        string storagePath = OperatingSystem.IsWindows()
            ? Import.Variables.vhdxHostDirectory
            : Import.Variables.installPath;
        long freeBytes = Partition_Manager.GetAvailableDriveBytes(storagePath);
        ...
        if (OperatingSystem.IsWindows())
        {
            Console.WriteLine(
                ...
                $"{Import.Variables.partitionDriveLetter}:.");
            Console.WriteLine("It will appear as a separate drive without changing the physical partition table.\n");
        }
        else
        {
            Console.WriteLine(
                "SurfOS will use a normal macOS directory with a logical capacity limit. " +
                "No disk image or partition is created.\n");
        }
    ...
    private static void ConfigureSecurity(int step, int totalSteps)
    {
        ...
        Console.WriteLine("Note: SurfOS is a simulator; this setting does not encrypt the host filesystem.");
    }
    ...
    private static bool ConfirmInstallation(int step, int totalSteps, bool guided)
    {
        ...
        {
            Console.WriteLine("Storage type:      Directory-backed virtual volume");
        }
        ...
        Console.WriteLine($"Filesystem:        {Import.Variables.partitionFileSystem} on " +
                          (OperatingSystem.IsWindows() ? "NTFS" : "the macOS host filesystem"));
        ...
        string storageDescription = OperatingSystem.IsWindows()
            ? $"fixed {Import.Variables.partitionSizeGb} GB SurfOS drive"
            : $"SurfOS directory with a {Import.Variables.partitionSizeGb} GB logical limit";
        return AskYesNo($"Create the {storageDescription} and install now?", true);
    }
    ...
    private static string ReadText(string prompt, string defaultValue, int maximumLength)
    {
        while (true)
        {
            ...
            if (value.Length <= maximumLength && PathSafety.IsSafeFileName(value))
            {
                return value;
            }
