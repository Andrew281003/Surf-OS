...
internal static class SurfOsApplication
{
    ...
    private static void RunBoot()
    {
        try
        {
            ...
            if (Partition_Manager.HasMountConfiguration)
            {
                ...
                if (vhdxInstallPath is null)
                {
                    ...
                    Console.ResetColor();
                    Console.WriteLine("The Windows partition table was not modified.");
                    return;
                }
            }

            string? targetPath = new string?[]
                { vhdxInstallPath, desktopPath, documentsPath, @"C:\SurfOS" }
