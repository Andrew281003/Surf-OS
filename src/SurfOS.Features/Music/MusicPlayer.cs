...
{
    private const int MaximumMusicDownloadBytes = 100 * 1024 * 1024;
    ...
    private static void Play(string requestedFile)
    {
        ...
        if (OperatingSystem.IsMacOS())
        {
            try
            {
                ProcessStartInfo startInfo = new("/usr/bin/afplay")
                {
                ...
                _audioProcess = Process.Start(startInfo)
                    ?? throw new InvalidOperationException("afplay could not be started.");
                _simulatedPlaying = false;
                KernelLog.Success("music", $"playing {_currentTrack} with afplay");
    ...
    private static void StopAudioProcess()
    {
        ...
        {
            // Playback already ended or the host reclaimed the process.
        }
        finally
        {
    ...
    private static void DownloadCloudSong(string songName)
    {
        ...
        {
            Console.WriteLine("SurfCloud music downloads are disabled by the current system profile.");
            return;
        }
        ...
        try
        {
            ...
            if (!Uri.TryCreate(songFileUrl, UriKind.Absolute, out Uri? songUri) ||
                songUri.Scheme != Uri.UriSchemeHttps)
            {
                throw new InvalidOperationException("Cloud music downloads require an HTTPS URL.");
            }
            if (song.FileSize is <= 0 or > MaximumMusicDownloadBytes)
            {
                throw new InvalidOperationException("Cloud song has an invalid or unsupported file size.");
            }

            string extension = Path.GetExtension(songUri.AbsolutePath).ToLowerInvariant();
            if (extension is not (".wav" or ".mp3" or ".ogg" or ".flac"))
            {
                throw new InvalidOperationException("Cloud song uses an unsupported file type.");
            }
            ...
            if (data.Length != song.FileSize || data.Length > MaximumMusicDownloadBytes)
            {
                throw new InvalidDataException("Downloaded song size does not match its manifest.");
            }
            File.WriteAllBytes(tempPath, data);
            if (!CloudRepositoryManager.ValidateSha256(tempPath, song.Sha256, out _))
            {
                throw new InvalidDataException("Downloaded song failed SHA-256 verification.");
            }
    ...
    private static void HandlePlaylistCommand(string[] args, int actionIndex)
    {
        ...
        if (action == "create" && args.Length > actionIndex + 1)
        {
            ...
            if (string.IsNullOrWhiteSpace(ToSafeFileName(name)))
            {
                Console.WriteLine("Playlist name must contain at least one valid filename character.");
                return;
            }
    ...
    private static CloudMusicLibrary? LoadCloudLibrary()
    {
        ...
        try
        {
            ...
            CloudMusicLibrary merged = MergeWithSeedLibrary(library);
            JsonStorage.Write(GetCloudCachePath(), merged);
            return merged;
        }
        catch (Exception ex)
        {
            ...
            if (File.Exists(GetCloudCachePath()))
            {
                try
                {
                    CloudMusicLibrary? cached = JsonStorage.Read<CloudMusicLibrary>(GetCloudCachePath());
                    return cached is null ? LoadSeedLibrary() : MergeWithSeedLibrary(cached);
                }
                catch
                {
                    return null;
    ...
    private static string ResolveMusicFile(string requestedFile)
    {
        ...
        string direct = Path.GetFullPath(Path.Combine(GetMusicPath(), requestedFile));
        if (!PathSafety.IsInsideRoot(direct, GetMusicPath()))
        {
            return string.Empty;
        }
    ...
    private static CloudMusicLibrary? LoadSeedLibrary()
    {
        string[] candidates =
        [
        ...
        foreach (string path in candidates.Distinct(StringComparer.OrdinalIgnoreCase))
        {
            try
            {
                if (File.Exists(path))
                {
                    return JsonStorage.Read<CloudMusicLibrary>(path);
                }
            }
    ...
    private static CloudMusicLibrary MergeWithSeedLibrary(CloudMusicLibrary library)
    {
        CloudMusicLibrary? seed = LoadSeedLibrary();
        if (seed is null)
        {
            return library;
        }
        ...
        foreach (CloudSong song in library.Songs)
        {
            CloudSong? seededSong = seed.Songs.FirstOrDefault(candidate =>
                candidate.Title.Equals(song.Title, StringComparison.OrdinalIgnoreCase));
            if (seededSong is not null)
            {
                if (string.IsNullOrWhiteSpace(song.Sha256)) song.Sha256 = seededSong.Sha256;
                if (song.FileSize <= 0) song.FileSize = seededSong.FileSize;
            }
        }

        foreach (CloudSong song in seed.Songs)
        {
            if (!library.Songs.Any(existing =>
                    existing.Title.Equals(song.Title, StringComparison.OrdinalIgnoreCase)))
            {
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine(OperatingSystem.IsMacOS()
            ? "Playback: supported audio files play through macOS afplay."
            : "Playback: WAV files play through Windows audio. MP3/OGG/FLAC are listed but simulated for now.");
    }
}
